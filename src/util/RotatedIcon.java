...
//
	...
	public int getIconWidth()
	{
		...
		{
			//Use input angle in degrees
			//double radians = Math.toRadians( angle );
			//Use input angle in radians
			double radians = angle;

        	double sin = Math.abs( Math.sin( radians ) );
        	double cos = Math.abs( Math.cos( radians ) );
			int width = (int)Math.ceil(icon.getIconWidth() * cos + icon.getIconHeight() * sin);
			return width;
		}
	...
	public int getIconHeight()
	{
		if (rotate == Rotate.ABOUT_CENTER)
		{
        	...
        	double sin = Math.abs( Math.sin( radians ) );
        	double cos = Math.abs( Math.cos( radians ) );
			int height = (int)Math.ceil(icon.getIconHeight() * cos + icon.getIconWidth() * sin);
			return height;
		}
		else if (rotate == Rotate.UPSIDE_DOWN)
			return icon.getIconHeight();
	...
	*/
	...
	public void paintIcon(Component c, Graphics g, int x, int y)
	{
		...
		else if (rotate == Rotate.ABOUT_CENTER)
		{
			...
			//Use input angle in radians
	        at.rotate(angle, x + cWidth, y + cHeight);
			g2.setTransform(at);
